...
pub struct EncryptionKey {
    ...
    pub id: Uuid,
    /// The actual key bytes (automatically zeroized on drop)
    #[zeroize(skip)]
    pub key: Vec<u8>,
...
impl CryptoService {
    ...
    pub async fn initialize_master_key(&self, password: &str, salt: Option<&[u8]>) -> Result<(), SecurityError> {
        ...
        let salt_obj = Salt::from_b64(&base64::encode(&salt))
    ...
    async fn encrypt_aes_256_gcm(&self, data: &[u8], classification: DataClassification, key_id: Option<Uuid>) -> Result<EncryptedData, SecurityError> {
        ...
        let aad = format!("PsyPsy-CMS-{}-{}", classification as u8, Utc::now().timestamp());
    ...
    async fn encrypt_medical_grade(&self, data: &[u8], classification: DataClassification, key_id: Option<Uuid>) -> Result<EncryptedData, SecurityError> {
        ...
        let mut result = self.encrypt_aes_256_gcm(data, classification, key_id).await?;
        result.algorithm = format!("Medical-Grade-AES-256-GCM-{:?}", classification);
        
        // Additional security: Re-encrypt key components with master key
        if let Some(master_key) = &*self.master_key.lock().await {
            let master_cipher_key = Key::<Aes256Gcm>::from_slice(&master_key[..32]);
            let master_cipher = Aes256Gcm::new(master_cipher_key);
            
            let mut master_nonce_bytes = [0u8; 12];
            self.rng.lock().await.fill_bytes(&mut master_nonce_bytes);
            let master_nonce = Nonce::from_slice(&master_nonce_bytes);
            
            // This is a placeholder for additional master key encryption
            // In practice, this would involve encrypting sensitive metadata
        }
    ...
    async fn encrypt_maximum_security(&self, data: &[u8], classification: DataClassification, key_id: Option<Uuid>) -> Result<EncryptedData, SecurityError> {
        ...
        let aes_result = self.encrypt_aes_256_gcm(&chacha_data, classification, key_id).await?;
